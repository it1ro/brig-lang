...
Ниже раздел для вставки в `docs/architecture.md`, затем оценка по подэтапам и список открытых вопросов. Код в разделе на Go. Сигнатуры и структуры — контракт, тела функций — ориентир для реализации 1:1. Я не компилировал этот код и не запускал тесты.

Дизайн опирается только на присланные файлы. Файлов `docs/*` за пределами присланного я не видел; это касается и текущей шапки `architecture.md`, которую я цитирую в §13 по присланному тексту.
...
Часть этих поведений в стековой VM «случайна». Они наблюдаемы, поэтому становятся контрактом.
...
### Сознательные исправления (единственные отклонения от стекового поведения)
...
**Почему 256 регистров.** Столько влезает в 8-битные `A/B/C`. Это же значение `MaxLocals` (Sprint 6.2 поднял его с 64 до 256 под реальные тесты). Новый лимит считает живые регистры (стековая дисциплина), а старый считал все слоты, когда-либо объявленные в функции. Для локалей и временных он строго мягче. Жёстче он только для широких вызовов и литералов: раньше их элементы жили на операнд-стеке до 65 535 штук, теперь им нужно окно регистров.
...
**Схема с `ensure`.** `E` (ошибка) и `F` (флаг «была ошибка») — временные выше `T`. Флаг `Bool` заменяет прежнее сравнение с атомом `:no_error` (D-5). Внешняя пара `TRAPBEGIN/TRAPEND` из стековой схемы не нужна: в её защищённом участке нет инструкций, способных бросить `*ErrRaise`.
...
### 10. Соответствие стековым опкодам
...
**Унаследованные ограничения** (не решаются дизайном, чтобы не смешивать с миграцией): K-8.
...
- В mermaid «Слои»: подпись `internal/vm` — «стековая ВМ + scheduler» → «регистровая ВМ + scheduler».
- Разделы «TCO и `ensure` (§15.3)» и «trap / ensure — схема байткода» заменяются разделом «Register VM design» (подразделы 4 и 5).
- В «Статус реализации»: `- [ ] **Регистровая VM**…` → `- [x]`; строку «Этап 4 (часть): стековая VM…» оставить (это история этапа).

Правки вне `architecture.md`:

- `README.md`: удалить пункт «Стек, а не регистры»; в структуре `vm/ # стековая ВМ` → `регистровая ВМ`.
- `STATUS.md`: закрыть чеклист Sprint 7; удалить процитированную в нём «Осознанное отступление от §15.1…».
- `cmd/brig/main.go`, `internal/vm/vm.go`, `internal/vm/opcodes.go`, `internal/compiler/compiler.go`: в комментариях-заголовках «стековая» → «регистровая».
