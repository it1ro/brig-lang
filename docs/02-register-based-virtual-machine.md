...
- **K-2. Условные переходы (строгий `Bool`, DD #41 = A).** Условие `if` и **оба** операнда `and`/`or` обязаны быть `Bool` (§7.2). `JMPIFNOT` прыгает на `Bool(false)`, `JMPIF` — на `Bool(true)`; на любом не-`Bool` они бросают ловимый `(:type_error, (:expected_bool, v))`. Truthiness нет: «проваливаться» дальше не-`Bool` не может. `and`/`or` всегда возвращают `Bool`. Правый операнд `and`/`or` проверяется после вычисления, поэтому он **не** в хвостовой позиции (§4, T-82 — won't-fix); хвостовая форма — `if c then true else loop(n - 1)`.
- **K-3. Что ловит `trap` (DD #42 = A).** Все `:type_error` ловимы: арифметика (включая унарный минус), `not` на не-`Bool`, `runtime.Compare`, не-`Bool` в условии (K-2), вызов не-функции, сравнение `Decimal` с `Float` (раздел «Сравнение чисел» ниже). Каждый из них — `*ErrRaise` с `(:type_error, (op, val))` и попадает в `trap` так же, как `raise`. Фатальны для актора только внутренние инварианты VM (сообщения с префиксом `internal:`, exit 3 в CLI). §10.4 спеки при этом не меняется: `:type_error` там уже перечислен среди авто-raise.
...
- **K-8 (вне рамок).** Мультиклозы `fn`, `when` в `recv` и захват в локальных `fn` реализованы. Не компилируются `match` (T-70, #114) и `with` (T-71, #122); вариадики — T-78 (#120). Дизайн это не чинит.
- **Пробелы вне K-8 (A-F8).** Следующие фичи из спеки не входят в список K-8, но в
  ...
  - pipe `|>` (§7.5) — T-72 (#115);
  - record-литералы — T-73 (#116);
  - `link` (§12.6) — T-75 (#117);
  - `Sys.args()` (§16 Must) — T-75 (#117);
  - `mailbox_size()` без аргументов (§12.6) — T-75 (#117).
...
| D-2 | Native с неверным числом аргументов → Go-panic (`args[0]`)                                                          | Фатальная ошибка `function_clause` (не catchable: внутренний инвариант VM, см. K-3)                                | Panic → диагностируемая ошибка                                     |
...
1. **Паттерны с числовым литералом — строго по виду.** Сопоставление требует того же `Kind` и точного значения (`runtime.MatchEqual`). Паттерн `1` не матчит `1.0` и `dec"1"`; `1.0` не матчит `dec"1.0"`.
2. **`==` и `<` на `Int × Float` — точное сравнение.** Без промежуточного `float64`: значения сравниваются как рациональные числа (`big.Rat`/`big.Float`), поэтому `2^53 + 1 == 2^53.0` ложно, а равенство транзитивно. `1 == 1.0` остаётся истиной. Быстрый путь допустим для значений, помещающихся в 53 бита. Особые значения: `NaN != NaN`, `NaN` не меньше и не больше никого; `+Inf` больше любого `Int`, `-Inf` меньше любого `Int`. Сортировка (`sort`), `Set` и `Map` опираются на то же сравнение.
3. **`Decimal × Float`.** В операторах `==`, `!=`, `<`, `>`, `<=`, `>=` — ловимый `(:type_error, (op, val))` (K-3). В паттернах и ключах `Map`/`Set` (в т.ч. `INDEX`, `Map.*`) сравнение строго по виду: `Decimal` и `Float` — разные значения, ошибки нет. `Decimal × Int` сравнивается по значению точно.

Пример: `trap(dec"1" < 1.0)` даёт `Error((:type_error, …))`, а `match 1.0` с веткой `dec"1.0"` не срабатывает и не бросает ошибку. Заметка: `trap(1 < "a")` — не пример type error: после T-77 это валидное сравнение по term order (§7.4), результат `true`.
