...
- A-F1 модель scheduler решён: вариант C (#40) — спека даёт только наблюдаемые гарантии G1–G4 (§15.2). Остальные design decisions ждут автора языка: A-F3 truthiness (#41), A-F4 ловится ли `:type_error` (#42), I-F8 равенство чисел (#43). Код в областях нерешённых DD не менять.
- Задачи T-81…T-86 (Wave 3) и T-62 (docs) ждут решений A-F3/A-F4/I-F8: issues #106–#112 в Blocked (T-80, #105 — по A-F1 — делается). После решения — в Todo или won't-fix по их DoD.
