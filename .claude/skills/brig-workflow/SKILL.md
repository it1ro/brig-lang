...
- A-F1 модель scheduler решён: вариант C (#40) — спека даёт только наблюдаемые гарантии G1–G4 (§15.2). A-F3 truthiness решён: вариант A (#41) — строгий `Bool` в `if` и в обоих операндах `and`/`or`, не-`Bool` → `:type_error`; правый операнд `and`/`or` не хвостовой (T-81 #106 делается, T-82 #107 — won't-fix по ветке A своего DoD). Остальные design decisions ждут автора языка: A-F4 ловится ли `:type_error` (#42), I-F8 равенство чисел (#43). Код в областях нерешённых DD не менять.
- Задачи T-83…T-86 (Wave 3) и T-62 (docs) ждут решений A-F4/I-F8: issues #108–#112 в Blocked (T-80, #105 — по A-F1 — делается; T-81 #106 и T-82 #107 — по A-F3 — в Todo). После решения — в Todo или won't-fix по их DoD.
