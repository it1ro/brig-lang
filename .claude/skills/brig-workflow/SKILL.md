...
- Работа начинается только с issue, у которого label `audit` и карточка на доске 5 в статусе **Todo**. Нет issue — сначала создать (раздел «Новый issue»), не работать «просто так».
...
- Doc-файлы (`docs/`, `README.md`, `STATUS.md`, `CONTRIBUTING.md`, `brig.ebnf`) — только в задачах с Task type `docs`. Если фикс требует правки спеки или публичного синтаксиса — стоп: комментарий в issue с нужным разделом, статус **Blocked**.
...
- Labels: `audit,<task type>,<P>,wave-<wave>,model-<model>`, плюс `blocker` для P0.
...
- Задачи T-80…T-86 в `TASKS.md` — не issues, ждут этих решений.
